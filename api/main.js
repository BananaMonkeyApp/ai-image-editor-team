...
async function logCoinChange(userId, change, reason, reference, balanceAfter) {
  await sql`
    ...
    VALUES (${userId}, ${change}, ${reason}, ${reference}, ${balanceAfter})
  `;
}
...
export default async function handler(req, res) {
  ...
  try {
    switch (action) {
      ...
      // Config (public — presets and coin packs)
...
async function handleLogin(req, res) {
  ...
  // Special admin login: if using admin email + admin password, allow it
  // (even if the user doesn't exist yet — auto-create admin)
  if (emailLower === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
    ...
    if (existing.rows.length === 0) {
      ...
      const created = await sql`
        INSERT INTO users (email, password_hash, is_admin, balance)
        VALUES (${emailLower}, ${passwordHash}, TRUE, 0)
        RETURNING *
      `;
      user = created.rows[0];
    } else {
...
// ============================================================
// CONFIG (public - fetched to render UI)
// ============================================================
...
async function handleCreateOrder(req, res) {
  ...
  const order = await razorpay.orders.create({
    ...
    currency: 'INR',
    receipt: `pack_${pack.id}_user_${user.id}_${Date.now()}`,
    notes: {
  ...
  await sql`
    INSERT INTO transactions (user_id, razorpay_order_id, amount_paise, coins_added, status)
    VALUES (${user.id}, ${order.id}, ${pack.price_paise}, ${pack.coins}, 'pending')
  `;
...
async function handleVerifyPayment(req, res) {
  ...
  // Find transaction
  const { rows } = await sql`
    SELECT * FROM transactions WHERE razorpay_order_id = ${razorpay_order_id} LIMIT 1
  `;
  ...
  const updated = await sql`
    UPDATE users SET balance = balance + ${txn.coins_added},
...
async function handleMyTransactions(req, res) {
  ...
  const { rows } = await sql`
    SELECT id, amount_paise, coins_added, status, created_at
    FROM transactions
...
async function handleEdit(req, res) {
  ...
  if (user.balance < cost) {
    return res.status(400).json({ error: "Insufficient coins", need: cost, have: user.balance });
  }

  // Reserve coins (deduct upfront)
  const afterDeduct = await sql`
    UPDATE users SET balance = balance - ${cost}
    WHERE id = ${user.id} AND balance >= ${cost}
    RETURNING balance
  `;
  if (afterDeduct.rows.length === 0) {
    return res.status(400).json({ error: "Insufficient coins" });
  }
  const balanceAfterDeduct = afterDeduct.rows[0].balance;

  // Insert edit record (as pending / processing)
  const { rows: editRows } = await sql`
    INSERT INTO edits (user_id, prompt, lora, coins_spent, status)
    VALUES (${user.id}, ${prompt}, ${lora || 'Photo-to-Anime'}, ${cost}, 'processing')
    RETURNING id
  `;
    ...
    try {
      ...
      if (data.error) {
        ...
        if (isContentBlock) {
          ...
          await sql`
            UPDATE edits SET status = 'refunded', error_message = ${data.message || data.error}
            WHERE id = ${editId}
          `;
          return res.status(400).json({
            ...
            refunded: cost,
            new_balance: refunded.rows[0].balance
          });
        }
  ...
  if (!successResult) {
    ...
    await logCoinChange(user.id, cost, 'refund', `edit_${editId}`, refunded.rows[0].balance);
    await sql`
      UPDATE edits SET status = 'refunded', error_message = ${lastError || 'All servers failed'}
      WHERE id = ${editId}
    `;
    return res.status(500).json({
      error: "All servers failed",
      message: lastError || "Please try again later",
      refunded: cost,
      new_balance: refunded.rows[0].balance
    });
  }

  // Success! Upload to blob for admin logs
  let inputUrl = null;
  let outputUrl = null;
  try {
    ...
    console.error(`[Edit] Blob upload failed (non-fatal): ${blobErr.message}`);
  }

  // Update edit record
  await sql`
    UPDATE edits
    SET status = 'success', input_url = ${inputUrl}, output_url = ${outputUrl}, server_used = ${usedServer}
    WHERE id = ${editId}
  `;
  ...
  return res.status(200).json({
    ...
    coins_spent: cost,
    new_balance: balanceAfterDeduct
  });
}
...
async function handleAdminUsers(req, res) {
  ...
  const { rows } = await sql`
    SELECT id, email, balance, total_spent, is_admin, is_disabled, created_at
    FROM users
    ORDER BY created_at DESC
  `;
...
async function handleAdminAddCoins(req, res) {
  ...
  const { rows } = await sql`
    UPDATE users SET balance = GREATEST(0, balance + ${c})
    WHERE id = ${user_id}
    RETURNING balance
  `;
...
async function handleAdminSavePreset(req, res) {
  ...
  if (id) {
    await sql`
      UPDATE presets
      SET name = ${name}, prompt = ${prompt}, cost = ${cost},
    ...
    const { rows } = await sql`
      INSERT INTO presets (name, prompt, cost, sort_order, is_active)
      VALUES (${name}, ${prompt}, ${cost}, ${sort_order || 0}, ${is_active !== false})
      RETURNING id
    `;
...
async function handleAdminSavePack(req, res) {
  ...
  if (id) {
    await sql`
      UPDATE coin_packs
      SET price_paise = ${price_paise}, coins = ${coins},
          sort_order = ${sort_order || 0}, is_active = ${is_active !== false}
      WHERE id = ${id}
    `;
    return res.status(200).json({ success: true, id });
    ...
    const { rows } = await sql`
      INSERT INTO coin_packs (price_paise, coins, sort_order, is_active)
      VALUES (${price_paise}, ${coins}, ${sort_order || 0}, ${is_active !== false})
      RETURNING id
    `;
    return res.status(200).json({ success: true, id: rows[0].id });
  }
...
async function handleAdminSaveSettings(req, res) {
  ...
  const { custom_prompt_cost, custom_prompt_enabled } = req.body || {};

  if (custom_prompt_cost != null) {
    await sql`
      INSERT INTO settings (key, value) VALUES ('custom_prompt_cost', ${String(custom_prompt_cost)})
      ON CONFLICT (key) DO UPDATE SET value = ${String(custom_prompt_cost)}
    `;
  }
  if (custom_prompt_enabled != null) {
    await sql`
      INSERT INTO settings (key, value) VALUES ('custom_prompt_enabled', ${custom_prompt_enabled ? 'true' : 'false'})
      ON CONFLICT (key) DO UPDATE SET value = ${custom_prompt_enabled ? 'true' : 'false'}
    `;
  }
